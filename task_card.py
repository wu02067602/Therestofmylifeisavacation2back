...
class TaskCardService:
    ...
    def create_card(
        ...
        Args:
            ...
            common_tasks (Sequence[str]): 常用任務列表
        ...
        serialized_common_tasks = self._serialize_common_tasks(common_tasks)
    ...
    def update_card(
        ...
        *,
        ...
        common_tasks: Optional[Sequence[str]] = None,
    ) -> TaskCardRecord:
        """
        更新既有任務卡。
        ...
        Args:
            ...
            common_tasks (Optional[Sequence[str]]): 新常用任務列表
        ...
        if common_tasks is not None:
            updates["common_tasks"] = self._serialize_common_tasks(common_tasks)
        ...
        record = self._database.update_task_card(
            ...
            common_tasks=updates.get("common_tasks"),
        )
        logger.info(
    ...
    def _serialize_common_tasks(self, common_tasks: Sequence[str]) -> str:
        """
        驗證並將常用任務序列化為 JSON。
        ...
        Examples:
             >>> service._serialize_common_tasks(["task1"]) # doctest: +SKIP
             '["task1"]'
        ...
        if common_tasks is None:
            raise ValueError("common_tasks 不可為 None")
        if isinstance(common_tasks, (str, bytes)):
             raise ValueError("common_tasks 必須為字串列表")
        
        normalized_items: list[str] = []
        for index, item in enumerate(common_tasks):
             if not isinstance(item, str):
                 raise ValueError(f"第 {index} 筆常用任務必須為字串")
             cleaned = item.strip()
             if cleaned:
